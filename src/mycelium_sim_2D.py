...
HEIGHT = 0.1
H0_PER_POINT = 10
SUBSTRATE_WIDTH = 5.0  # mm, width of substrate in Y direction

dist_inoculum = 0.5  # mm, distance between multiple inoculum points

def generate_inoculum_grid(nx, ny, dist, z=0.0, centered=True):
    """
    Generate a rectangular grid of inoculum points in the XY plane.
    
    nx: number of points along x direction
    ny: number of points along y direction
    dist: spacing between points
    z: fixed z-coordinate (default 0 → XY plane)
    centered: if True, grid is centered at (0,0)
    """
...
dist_inoculum = 0.5
INOCULUM_POINTS = generate_inoculum_grid(nx=5, ny=5, dist=dist_inoculum)
...
def sph_to_cart(theta, phi):
    """
    Returns a 2D direction (x,y,0) because theta = pi/2 always.
    """
    return np.array([math.cos(phi), math.sin(phi), 0.0])

def rand_direction_from(theta, phi, lam=lambda_angle):
    """
    2D version: keep everything in XY plane.
    theta is ignored (set to 90°), only phi is perturbed.
    """
    dph = (random.random() - 0.5) * lam
    phi_new = phi + dph
    theta_new = math.pi / 2   # always horizontal
...
# Visualization
# -------------------------
def plot_mycelium(M, step, cuboids=None, title=None, show=True, snapshot_dir=None, dish_size=DISH_SIZE):
    if snapshot_dir is None:
        snapshot_dir = SNAPSHOT_DIR
    os.makedirs(snapshot_dir, exist_ok=True)

    fig, ax = plt.subplots(figsize=(7,7))   # 2D now, faster

    # --- collect segments ---
    segments = []
    intensities = []

    # Precompute max intensity for color normalization
    seg_list = list(M.all_segments())
    if len(seg_list) > 0:
        max_val = max((s.I * s.length()) for _,_,s in seg_list)
        if max_val < 1e-12:
            max_val = 1.0
    else:
        max_val = 1.0

    for _,_,s in seg_list:
        p1 = s.start
        p2 = s.end
        segments.append([[p1[0], p1[1]], [p2[0], p2[1]]])
        intensities.append((s.I * s.length()) / max_val)

    intensities = np.array(intensities)

    # --- LineCollection for segments ---
    if len(segments) > 0:
        lc = LineCollection(
            segments,
            cmap="viridis",
    ...
    # --- plot cuboids (projected onto XY) ---
    # if cuboids:
    #     for c in cuboids:
    #         draw_cuboid_2D(ax, c)  # You will need a 2D version (I can write it)
    ...
    plt.xlim(-dish_size/2, dish_size/2)
    plt.ylim(-dish_size/2, dish_size/2)
...
def run_demo():
    ...
    cuboids += [
        Cuboid(center=[0, DISH_SIZE/2 + WALL_THICKNESS/2, 0],
               size=[DISH_SIZE, WALL_THICKNESS, WALL_THICKNESS],
               ctype='impenetrable'),
        Cuboid(center=[0, -DISH_SIZE/2 - WALL_THICKNESS/2, 0],
               size=[DISH_SIZE, WALL_THICKNESS, WALL_THICKNESS],
               ctype='impenetrable'),
        Cuboid(center=[DISH_SIZE/2 + WALL_THICKNESS/2, 0, 0],
               size=[WALL_THICKNESS, DISH_SIZE, WALL_THICKNESS],
               ctype='impenetrable'),
        Cuboid(center=[-DISH_SIZE/2 - WALL_THICKNESS/2, 0, 0],
               size=[WALL_THICKNESS, DISH_SIZE, WALL_THICKNESS],
               ctype='impenetrable')
    ...
    for t in range(T_steps):
        ...
        if t % 1 == 0 or t == T_steps - 1:
            ...
            plot_mycelium(M, t, cuboids=cuboids, title=f"Step {t} (Petri Dish)", show=False, snapshot_dir=snapshot_dir, dish_size=DISH_SIZE)
