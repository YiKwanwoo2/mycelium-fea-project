...
int main(int argc, char **argv) {
    ...
    std::ofstream runtime_csv;
    runtime_csv.open(results_dir + "/runtime_results.csv", std::ios::app);
    runtime_csv << "step,solver,pc,trial,runtime_ms\n";
    ...
    for (int step=0; step < N_STEPS; ++step) {
        ...
        // Solve K * U = b
        // ===============================================
        // ITERATE OVER SOLVER × PC combinations
        // ===============================================
        std::vector<std::string> solvers = {"cg", "bcgs", "gmres"};
        std::vector<std::string> pcs     = {"jacobi", "sor", "ilu", "icc", "gamg"};
        ...
        for (const auto& solver : solvers) {
            for (const auto& prec : pcs) {
                for (int trial = 1; trial <= 5; ++trial) {
                    ...
                    ierr = PCSetType(pc, prec.c_str()); CHKERRQ(ierr);
                    ...
                    // Solve
                    Vec Uloc;
                    ierr = VecDuplicate(bvec, &Uloc); CHKERRQ(ierr);
                    ierr = VecSet(Uloc, 0.0); CHKERRQ(ierr);

                    ierr = KSPSolve(ksp, bvec, Uloc); CHKERRQ(ierr);
                    ...
                    ierr = VecDuplicate(Uloc, &F_react); CHKERRQ(ierr);
                    ierr = MatMult(K_orig, Uloc, F_react); CHKERRQ(ierr);
                    
                    if (solver == solvers.back() && prec == pcs.back() && trial == 5) {
                        ierr = VecDuplicate(Uloc, &U); CHKERRQ(ierr);
                        ierr = VecCopy(Uloc, U); CHKERRQ(ierr);
                    }
